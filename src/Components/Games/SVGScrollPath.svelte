<script lang="ts">
    ...
    const heightComponent: number = (max_scrollbar_height * (endPercentage - startPercentage)) / 100;

    window.addEventListener("scroll", () => {
        percentageCompleted = (YPosition - (startPercentage * max_scrollbar_height) / 100) / heightComponent;
        if (percentageCompleted < 0 || percentageCompleted > 1) percentageCompleted = 0;
    });
    onMount(() => {
