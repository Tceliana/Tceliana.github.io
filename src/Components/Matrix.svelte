<ul class="matrix">
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
</ul>
<style>
@charset "UTF-8";
.matrix 
{
    z-index: -1;
    background:     black;
    overflow:       hidden;
    margin:         0;
    padding:        0;
    text-align:     center;
}

.matrix 
{
    display:        block;
    list-style:     none;
    padding:        0;
    margin:         0;
    color:          #80ff80;
    font-size:      18px;
    line-height:    24px;
    overflow:       hidden;
    width:          100%;
    white-space:    nowrap;
}

.matrix li:nth-child(1) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(1):before {
  display: block;
  width: 1em;
  content: "シ ン ク こ ば う ソ マ  T C E L A タ ラ コ た ワ た ラ タ";
}
.matrix li:nth-child(1):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -504px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #FFFFFF), color-stop(95.83333%, rgba(0, 0, 0, 0)), color-stop(95.83333%, #000000), color-stop(100%, #000000)) 0 -504px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(255, 0, 0, 0) 552px, #FFFFFF 552px, #000000 576px) 0 -504px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 255, 0, 0) 552px, #000000 552px, #000000 576px) 0 -504px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 255, 0) 552px, #000000 552px, #000000 576px) 0 -504px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ク";
  padding-top: 48px;
  overflow: hidden;
  -webkit-animation: 1920ms column-1 infinite steps(1, start);
  -moz-animation: 1920ms column-1 infinite steps(1, start);
  -ms-animation: 1920ms column-1 infinite steps(1, start);
  -o-animation: 1920ms column-1 infinite steps(1, start);
  animation: 1920ms column-1 infinite steps(1, start);
}

@-webkit-keyframes column-1 {
  0% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
}
@-moz-keyframes column-1 {
  0% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
}
@-o-keyframes column-1 {
  0% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
}
@keyframes column-1 {
  0% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
}
.matrix li:nth-child(2) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(2):before {
  display: block;
  width: 1em;
  content: "ウ ョ タ ソ フ ば ソ ド ョ ド ょ ば W I L M I タ ョ シ";
}
.matrix li:nth-child(2):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(58.33333%, rgba(0, 0, 0, 0)), color-stop(58.33333%, #000000), color-stop(100%, #000000)) 0 -24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ョ";
  padding-top: 312px;
  overflow: hidden;
  -webkit-animation: 1920ms column-2 infinite steps(1, start);
  -moz-animation: 1920ms column-2 infinite steps(1, start);
  -ms-animation: 1920ms column-2 infinite steps(1, start);
  -o-animation: 1920ms column-2 infinite steps(1, start);
  animation: 1920ms column-2 infinite steps(1, start);
}

@-webkit-keyframes column-2 {
  0% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
}
@-moz-keyframes column-2 {
  0% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
}
@-o-keyframes column-2 {
  0% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
}
@keyframes column-2 {
  0% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
}
.matrix li:nth-child(3) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(3):before {
  display: block;
  width: 1em;
  content: "き W A Y R A う う ワ W A Y R A た こ ょ W A Y R A き";
}
.matrix li:nth-child(3):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(16.66667%, rgba(0, 0, 0, 0)), color-stop(16.66667%, #000000), color-stop(100%, #000000)) 0 24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ょ";
  padding-top: 120px;
  overflow: hidden;
  -webkit-animation: 1920ms column-3 infinite steps(1, start);
  -moz-animation: 1920ms column-3 infinite steps(1, start);
  -ms-animation: 1920ms column-3 infinite steps(1, start);
  -o-animation: 1920ms column-3 infinite steps(1, start);
  animation: 1920ms column-3 infinite steps(1, start);
}

@-webkit-keyframes column-3 {
  0% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  45.83333% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  50% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  54.16667% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  58.33333% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  62.5% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  66.66667% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  70.83333% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  75% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 96px;
  }
}
@-moz-keyframes column-3 {
  0% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  45.83333% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  50% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  54.16667% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  58.33333% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  62.5% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  66.66667% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  70.83333% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  75% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 96px;
  }
}
@-o-keyframes column-3 {
  0% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  45.83333% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  50% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  54.16667% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  58.33333% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  62.5% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  66.66667% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  70.83333% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  75% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 96px;
  }
}
@keyframes column-3 {
  0% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  45.83333% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  50% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  54.16667% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  58.33333% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  62.5% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  66.66667% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  70.83333% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  75% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 96px;
  }
}
.matrix li:nth-child(4) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(4):before {
  display: block;
  width: 1em;
  content: "シ コ フ M U S H Uト ド し わ フ M U S H Uコ ん う ば";
}
.matrix li:nth-child(4):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 408px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(20.83333%, rgba(0, 0, 0, 0)), color-stop(20.83333%, #000000), color-stop(100%, #000000)) 0 408px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 408px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 408px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 408px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "う";
  padding-top: 528px;
  overflow: hidden;
  -webkit-animation: 1920ms column-4 infinite steps(1, start);
  -moz-animation: 1920ms column-4 infinite steps(1, start);
  -ms-animation: 1920ms column-4 infinite steps(1, start);
  -o-animation: 1920ms column-4 infinite steps(1, start);
  animation: 1920ms column-4 infinite steps(1, start);
}

@-webkit-keyframes column-4 {
  0% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 504px;
  }
}
@-moz-keyframes column-4 {
  0% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 504px;
  }
}
@-o-keyframes column-4 {
  0% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 504px;
  }
}
@keyframes column-4 {
  0% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 504px;
  }
}
.matrix li:nth-child(5) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(5):before {
  display: block;
  width: 1em;
  content: "ョ バ シ と バ マ フ ク し ょ ょ ョ ョ と ョ ワ ョ ョ ド ん た ウ ン タ";
}
.matrix li:nth-child(5):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 96px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(29.16667%, rgba(0, 0, 0, 0)), color-stop(29.16667%, #000000), color-stop(100%, #000000)) 0 96px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 96px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 96px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 96px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ョ";
  padding-top: 264px;
  overflow: hidden;
  -webkit-animation: 1920ms column-5 infinite steps(1, start);
  -moz-animation: 1920ms column-5 infinite steps(1, start);
  -ms-animation: 1920ms column-5 infinite steps(1, start);
  -o-animation: 1920ms column-5 infinite steps(1, start);
  animation: 1920ms column-5 infinite steps(1, start);
}

@-webkit-keyframes column-5 {
  0% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 240px;
  }
}
@-moz-keyframes column-5 {
  0% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 240px;
  }
}
@-o-keyframes column-5 {
  0% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 240px;
  }
}
@keyframes column-5 {
  0% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 240px;
  }
}
.matrix li:nth-child(6) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(6):before {
  display: block;
  width: 1em;
  content: "ソ マ わ バ ょ ョ マ ょ ン ン こ ョ き リ バ ウ ド し キ ん ウ ン ョ ョ";
}
.matrix li:nth-child(6):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -216px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(54.16667%, rgba(0, 0, 0, 0)), color-stop(54.16667%, #000000), color-stop(100%, #000000)) 0 -216px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 -216px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 -216px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 -216px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ょ";
  padding-top: 96px;
  overflow: hidden;
  -webkit-animation: 1920ms column-6 infinite steps(1, start);
  -moz-animation: 1920ms column-6 infinite steps(1, start);
  -ms-animation: 1920ms column-6 infinite steps(1, start);
  -o-animation: 1920ms column-6 infinite steps(1, start);
  animation: 1920ms column-6 infinite steps(1, start);
}

@-webkit-keyframes column-6 {
  0% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 216px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
}
@-moz-keyframes column-6 {
  0% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 216px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
}
@-o-keyframes column-6 {
  0% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 216px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
}
@keyframes column-6 {
  0% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 216px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
}
.matrix li:nth-child(7) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(7):before {
  display: block;
  width: 1em;
  content: "マ こ た う こ キ シ ン ド ン ョ わ シ ソ ウ ウ た バ こ ン こ リ ん し";
}
.matrix li:nth-child(7):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 120px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(45.83333%, rgba(0, 0, 0, 0)), color-stop(45.83333%, #000000), color-stop(100%, #000000)) 0 120px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 120px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 120px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 120px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "た";
  padding-top: 384px;
  overflow: hidden;
  -webkit-animation: 1920ms column-7 infinite steps(1, start);
  -moz-animation: 1920ms column-7 infinite steps(1, start);
  -ms-animation: 1920ms column-7 infinite steps(1, start);
  -o-animation: 1920ms column-7 infinite steps(1, start);
  animation: 1920ms column-7 infinite steps(1, start);
}

@-webkit-keyframes column-7 {
  0% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  4.16667% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  8.33333% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  12.5% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  16.66667% {
    background-position: 0 216px;
    padding-top: 480px;
  }
  20.83333% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  25% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  29.16667% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  33.33333% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  37.5% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  41.66667% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  45.83333% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  50% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  54.16667% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  58.33333% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  62.5% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  66.66667% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  70.83333% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  75% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  79.16667% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  83.33333% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  87.5% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  91.66667% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  95.83333% {
    background-position: 0 96px;
    padding-top: 360px;
  }
}
@-moz-keyframes column-7 {
  0% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  4.16667% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  8.33333% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  12.5% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  16.66667% {
    background-position: 0 216px;
    padding-top: 480px;
  }
  20.83333% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  25% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  29.16667% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  33.33333% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  37.5% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  41.66667% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  45.83333% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  50% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  54.16667% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  58.33333% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  62.5% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  66.66667% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  70.83333% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  75% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  79.16667% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  83.33333% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  87.5% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  91.66667% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  95.83333% {
    background-position: 0 96px;
    padding-top: 360px;
  }
}
@-o-keyframes column-7 {
  0% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  4.16667% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  8.33333% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  12.5% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  16.66667% {
    background-position: 0 216px;
    padding-top: 480px;
  }
  20.83333% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  25% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  29.16667% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  33.33333% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  37.5% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  41.66667% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  45.83333% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  50% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  54.16667% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  58.33333% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  62.5% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  66.66667% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  70.83333% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  75% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  79.16667% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  83.33333% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  87.5% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  91.66667% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  95.83333% {
    background-position: 0 96px;
    padding-top: 360px;
  }
}
@keyframes column-7 {
  0% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  4.16667% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  8.33333% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  12.5% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  16.66667% {
    background-position: 0 216px;
    padding-top: 480px;
  }
  20.83333% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  25% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  29.16667% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  33.33333% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  37.5% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  41.66667% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  45.83333% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  50% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  54.16667% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  58.33333% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  62.5% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  66.66667% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  70.83333% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  75% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  79.16667% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  83.33333% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  87.5% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  91.66667% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  95.83333% {
    background-position: 0 96px;
    padding-top: 360px;
  }
}
.matrix li:nth-child(8) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(8):before {
  display: block;
  width: 1em;
  content: "わ ウ ょ と ワ こ リ ン タ ょ キ シ ド わ ラ ト ト ョ う マ ば こ ョ し";
}
.matrix li:nth-child(8):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 72px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(66.66667%, rgba(0, 0, 0, 0)), color-stop(66.66667%, #000000), color-stop(100%, #000000)) 0 72px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 72px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 72px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 72px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "マ";
  padding-top: 456px;
  overflow: hidden;
  -webkit-animation: 1920ms column-8 infinite steps(1, start);
  -moz-animation: 1920ms column-8 infinite steps(1, start);
  -ms-animation: 1920ms column-8 infinite steps(1, start);
  -o-animation: 1920ms column-8 infinite steps(1, start);
  animation: 1920ms column-8 infinite steps(1, start);
}

@-webkit-keyframes column-8 {
  0% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 48px;
    padding-top: 432px;
  }
}
@-moz-keyframes column-8 {
  0% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 48px;
    padding-top: 432px;
  }
}
@-o-keyframes column-8 {
  0% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 48px;
    padding-top: 432px;
  }
}
@keyframes column-8 {
  0% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 48px;
    padding-top: 432px;
  }
}
.matrix li:nth-child(9) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(9):before {
  display: block;
  width: 1em;
  content: "ょ ト シ ラ ん バ ン ラ わ タ シ ク う キ ン ウ ょ う ョ う ン ン ク フ";
}
.matrix li:nth-child(9):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -288px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(75%, rgba(0, 0, 0, 0)), color-stop(75%, #000000), color-stop(100%, #000000)) 0 -288px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -288px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -288px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -288px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ン";
  padding-top: 144px;
  overflow: hidden;
  -webkit-animation: 1920ms column-9 infinite steps(1, start);
  -moz-animation: 1920ms column-9 infinite steps(1, start);
  -ms-animation: 1920ms column-9 infinite steps(1, start);
  -o-animation: 1920ms column-9 infinite steps(1, start);
  animation: 1920ms column-9 infinite steps(1, start);
}

@-webkit-keyframes column-9 {
  0% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
}
@-moz-keyframes column-9 {
  0% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
}
@-o-keyframes column-9 {
  0% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
}
@keyframes column-9 {
  0% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
}
.matrix li:nth-child(10) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(10):before {
  display: block;
  width: 1em;
  content: "ラ ド タ ウ ク ラ う ラ ョ マ き フ シ ト ょ ウ わ こ ン リ バ バ キ ょ";
}
.matrix li:nth-child(10):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 0px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(79.16667%, rgba(0, 0, 0, 0)), color-stop(79.16667%, #000000), color-stop(100%, #000000)) 0 0px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 456px, #000000 456px, #000000 576px) 0 0px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 456px, #000000 456px, #000000 576px) 0 0px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 456px, #000000 456px, #000000 576px) 0 0px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "リ";
  padding-top: 456px;
  overflow: hidden;
  -webkit-animation: 1920ms column-10 infinite steps(1, start);
  -moz-animation: 1920ms column-10 infinite steps(1, start);
  -ms-animation: 1920ms column-10 infinite steps(1, start);
  -o-animation: 1920ms column-10 infinite steps(1, start);
  animation: 1920ms column-10 infinite steps(1, start);
}

@-webkit-keyframes column-10 {
  0% {
    background-position: 0 0px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 24px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 48px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 72px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 96px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -456px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -432px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -408px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -384px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -360px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -336px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -312px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -288px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -264px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -240px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -216px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -192px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -168px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -144px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -120px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -96px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 -72px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 -48px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 -24px;
    padding-top: 432px;
  }
}
@-moz-keyframes column-10 {
  0% {
    background-position: 0 0px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 24px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 48px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 72px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 96px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -456px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -432px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -408px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -384px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -360px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -336px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -312px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -288px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -264px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -240px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -216px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -192px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -168px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -144px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -120px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -96px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 -72px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 -48px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 -24px;
    padding-top: 432px;
  }
}
@-o-keyframes column-10 {
  0% {
    background-position: 0 0px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 24px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 48px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 72px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 96px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -456px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -432px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -408px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -384px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -360px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -336px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -312px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -288px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -264px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -240px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -216px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -192px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -168px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -144px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -120px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -96px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 -72px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 -48px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 -24px;
    padding-top: 432px;
  }
}
@keyframes column-10 {
  0% {
    background-position: 0 0px;
    padding-top: 456px;
  }
  4.16667% {
    background-position: 0 24px;
    padding-top: 480px;
  }
  8.33333% {
    background-position: 0 48px;
    padding-top: 504px;
  }
  12.5% {
    background-position: 0 72px;
    padding-top: 528px;
  }
  16.66667% {
    background-position: 0 96px;
    padding-top: 552px;
  }
  20.83333% {
    background-position: 0 -456px;
    padding-top: 0px;
  }
  25% {
    background-position: 0 -432px;
    padding-top: 24px;
  }
  29.16667% {
    background-position: 0 -408px;
    padding-top: 48px;
  }
  33.33333% {
    background-position: 0 -384px;
    padding-top: 72px;
  }
  37.5% {
    background-position: 0 -360px;
    padding-top: 96px;
  }
  41.66667% {
    background-position: 0 -336px;
    padding-top: 120px;
  }
  45.83333% {
    background-position: 0 -312px;
    padding-top: 144px;
  }
  50% {
    background-position: 0 -288px;
    padding-top: 168px;
  }
  54.16667% {
    background-position: 0 -264px;
    padding-top: 192px;
  }
  58.33333% {
    background-position: 0 -240px;
    padding-top: 216px;
  }
  62.5% {
    background-position: 0 -216px;
    padding-top: 240px;
  }
  66.66667% {
    background-position: 0 -192px;
    padding-top: 264px;
  }
  70.83333% {
    background-position: 0 -168px;
    padding-top: 288px;
  }
  75% {
    background-position: 0 -144px;
    padding-top: 312px;
  }
  79.16667% {
    background-position: 0 -120px;
    padding-top: 336px;
  }
  83.33333% {
    background-position: 0 -96px;
    padding-top: 360px;
  }
  87.5% {
    background-position: 0 -72px;
    padding-top: 384px;
  }
  91.66667% {
    background-position: 0 -48px;
    padding-top: 408px;
  }
  95.83333% {
    background-position: 0 -24px;
    padding-top: 432px;
  }
}
.matrix li:nth-child(11) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(11):before {
  display: block;
  width: 1em;
  content: "バ ん ん バ ウ わ ド ば ン ン う ん シ ょ ョ ワ キ わ フ ト こ ょ わ ウ";
}
.matrix li:nth-child(11):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 48px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(83.33333%, rgba(0, 0, 0, 0)), color-stop(83.33333%, #000000), color-stop(100%, #000000)) 0 48px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 480px, #000000 480px, #000000 576px) 0 48px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 480px, #000000 480px, #000000 576px) 0 48px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 480px, #000000 480px, #000000 576px) 0 48px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "わ";
  padding-top: 528px;
  overflow: hidden;
  -webkit-animation: 1920ms column-11 infinite steps(1, start);
  -moz-animation: 1920ms column-11 infinite steps(1, start);
  -ms-animation: 1920ms column-11 infinite steps(1, start);
  -o-animation: 1920ms column-11 infinite steps(1, start);
  animation: 1920ms column-11 infinite steps(1, start);
}

@-webkit-keyframes column-11 {
  0% {
    background-position: 0 48px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 72px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -480px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -456px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -432px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -408px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -384px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -360px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -336px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -312px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -288px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -264px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -240px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -216px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -192px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -168px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -144px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -120px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -96px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -72px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -48px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -24px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 0px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 24px;
    padding-top: 504px;
  }
}
@-moz-keyframes column-11 {
  0% {
    background-position: 0 48px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 72px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -480px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -456px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -432px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -408px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -384px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -360px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -336px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -312px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -288px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -264px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -240px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -216px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -192px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -168px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -144px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -120px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -96px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -72px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -48px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -24px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 0px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 24px;
    padding-top: 504px;
  }
}
@-o-keyframes column-11 {
  0% {
    background-position: 0 48px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 72px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -480px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -456px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -432px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -408px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -384px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -360px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -336px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -312px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -288px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -264px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -240px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -216px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -192px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -168px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -144px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -120px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -96px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -72px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -48px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -24px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 0px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 24px;
    padding-top: 504px;
  }
}
@keyframes column-11 {
  0% {
    background-position: 0 48px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 72px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -480px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -456px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -432px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -408px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -384px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -360px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -336px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -312px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -288px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -264px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -240px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -216px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -192px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -168px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -144px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -120px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -96px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -72px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -48px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -24px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 0px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 24px;
    padding-top: 504px;
  }
}
.matrix li:nth-child(12) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(12):before {
  display: block;
  width: 1em;
  content: "ウ と ん ド ウ ソ ョ ク ウ ト ョ ょ し ト ワ ト ん ョ た ウ コ た ば タ";
}
.matrix li:nth-child(12):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 408px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(25%, rgba(0, 0, 0, 0)), color-stop(25%, #000000), color-stop(100%, #000000)) 0 408px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 144px, #000000 144px, #000000 576px) 0 408px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 144px, #000000 144px, #000000 576px) 0 408px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 144px, #000000 144px, #000000 576px) 0 408px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "タ";
  padding-top: 552px;
  overflow: hidden;
  -webkit-animation: 1920ms column-12 infinite steps(1, start);
  -moz-animation: 1920ms column-12 infinite steps(1, start);
  -ms-animation: 1920ms column-12 infinite steps(1, start);
  -o-animation: 1920ms column-12 infinite steps(1, start);
  animation: 1920ms column-12 infinite steps(1, start);
}

@-webkit-keyframes column-12 {
  0% {
    background-position: 0 408px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 528px;
  }
}
@-moz-keyframes column-12 {
  0% {
    background-position: 0 408px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 528px;
  }
}
@-o-keyframes column-12 {
  0% {
    background-position: 0 408px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 528px;
  }
}
@keyframes column-12 {
  0% {
    background-position: 0 408px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 192px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 216px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 240px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 264px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 288px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 312px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 336px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 360px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 384px;
    padding-top: 528px;
  }
}
.matrix li:nth-child(13) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(13):before {
  display: block;
  width: 1em;
  content: "ょ ウ タ フ タ こ し ソ き ン バ タ ド し タ タ ば ょ バ ば わ ク コ ウ";
}
.matrix li:nth-child(13):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -336px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(87.5%, rgba(0, 0, 0, 0)), color-stop(87.5%, #000000), color-stop(100%, #000000)) 0 -336px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -336px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -336px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -336px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ソ";
  padding-top: 168px;
  overflow: hidden;
  -webkit-animation: 1920ms column-13 infinite steps(1, start);
  -moz-animation: 1920ms column-13 infinite steps(1, start);
  -ms-animation: 1920ms column-13 infinite steps(1, start);
  -o-animation: 1920ms column-13 infinite steps(1, start);
  animation: 1920ms column-13 infinite steps(1, start);
}

@-webkit-keyframes column-13 {
  0% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  70.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  75% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  79.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  83.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  87.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  91.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  95.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
}
@-moz-keyframes column-13 {
  0% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  70.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  75% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  79.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  83.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  87.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  91.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  95.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
}
@-o-keyframes column-13 {
  0% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  70.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  75% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  79.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  83.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  87.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  91.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  95.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
}
@keyframes column-13 {
  0% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  70.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  75% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  79.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  83.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  87.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  91.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  95.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
}
.matrix li:nth-child(14) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(14):before {
  display: block;
  width: 1em;
  content: "ン フ ウ ん フ キ キ と ョ ョ ウ う こ ウ ば う リ ン ョ ソ ょ ん コ ラ";
}
.matrix li:nth-child(14):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(50%, rgba(0, 0, 0, 0)), color-stop(50%, #000000), color-stop(100%, #000000)) 0 24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 288px, #000000 288px, #000000 576px) 0 24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 288px, #000000 288px, #000000 576px) 0 24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 288px, #000000 288px, #000000 576px) 0 24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ウ";
  padding-top: 312px;
  overflow: hidden;
  -webkit-animation: 1920ms column-14 infinite steps(1, start);
  -moz-animation: 1920ms column-14 infinite steps(1, start);
  -ms-animation: 1920ms column-14 infinite steps(1, start);
  -o-animation: 1920ms column-14 infinite steps(1, start);
  animation: 1920ms column-14 infinite steps(1, start);
}

@-webkit-keyframes column-14 {
  0% {
    background-position: 0 24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -288px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -264px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -240px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -216px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -192px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -168px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -144px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -120px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 288px;
  }
}
@-moz-keyframes column-14 {
  0% {
    background-position: 0 24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -288px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -264px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -240px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -216px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -192px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -168px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -144px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -120px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 288px;
  }
}
@-o-keyframes column-14 {
  0% {
    background-position: 0 24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -288px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -264px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -240px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -216px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -192px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -168px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -144px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -120px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 288px;
  }
}
@keyframes column-14 {
  0% {
    background-position: 0 24px;
    padding-top: 312px;
  }
  4.16667% {
    background-position: 0 48px;
    padding-top: 336px;
  }
  8.33333% {
    background-position: 0 72px;
    padding-top: 360px;
  }
  12.5% {
    background-position: 0 96px;
    padding-top: 384px;
  }
  16.66667% {
    background-position: 0 120px;
    padding-top: 408px;
  }
  20.83333% {
    background-position: 0 144px;
    padding-top: 432px;
  }
  25% {
    background-position: 0 168px;
    padding-top: 456px;
  }
  29.16667% {
    background-position: 0 192px;
    padding-top: 480px;
  }
  33.33333% {
    background-position: 0 216px;
    padding-top: 504px;
  }
  37.5% {
    background-position: 0 240px;
    padding-top: 528px;
  }
  41.66667% {
    background-position: 0 264px;
    padding-top: 552px;
  }
  45.83333% {
    background-position: 0 -288px;
    padding-top: 0px;
  }
  50% {
    background-position: 0 -264px;
    padding-top: 24px;
  }
  54.16667% {
    background-position: 0 -240px;
    padding-top: 48px;
  }
  58.33333% {
    background-position: 0 -216px;
    padding-top: 72px;
  }
  62.5% {
    background-position: 0 -192px;
    padding-top: 96px;
  }
  66.66667% {
    background-position: 0 -168px;
    padding-top: 120px;
  }
  70.83333% {
    background-position: 0 -144px;
    padding-top: 144px;
  }
  75% {
    background-position: 0 -120px;
    padding-top: 168px;
  }
  79.16667% {
    background-position: 0 -96px;
    padding-top: 192px;
  }
  83.33333% {
    background-position: 0 -72px;
    padding-top: 216px;
  }
  87.5% {
    background-position: 0 -48px;
    padding-top: 240px;
  }
  91.66667% {
    background-position: 0 -24px;
    padding-top: 264px;
  }
  95.83333% {
    background-position: 0 0px;
    padding-top: 288px;
  }
}
.matrix li:nth-child(15) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(15):before {
  display: block;
  width: 1em;
  content: "ラ ト タ ョ キ し わ た ラ と シ タ ソ マ ョ ウ マ ド ク キ リ ク き マ";
}
.matrix li:nth-child(15):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -48px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(20.83333%, rgba(0, 0, 0, 0)), color-stop(20.83333%, #000000), color-stop(100%, #000000)) 0 -48px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 -48px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 -48px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 120px, #000000 120px, #000000 576px) 0 -48px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ョ";
  padding-top: 72px;
  overflow: hidden;
  -webkit-animation: 1920ms column-15 infinite steps(1, start);
  -moz-animation: 1920ms column-15 infinite steps(1, start);
  -ms-animation: 1920ms column-15 infinite steps(1, start);
  -o-animation: 1920ms column-15 infinite steps(1, start);
  animation: 1920ms column-15 infinite steps(1, start);
}

@-webkit-keyframes column-15 {
  0% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 384px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
}
@-moz-keyframes column-15 {
  0% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 384px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
}
@-o-keyframes column-15 {
  0% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 384px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
}
@keyframes column-15 {
  0% {
    background-position: 0 -48px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -24px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 0px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 24px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 48px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 72px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 96px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 144px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 168px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 216px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 240px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 264px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 288px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 312px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 336px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 360px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 384px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 408px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 432px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -120px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -96px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -72px;
    padding-top: 48px;
  }
}
.matrix li:nth-child(16) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(16):before {
  display: block;
  width: 1em;
  content: "う ョ コ ウ シ ン わ キ ウ ョ ト わ ウ フ わ ょ し ウ と き ド ク ン ド";
}
.matrix li:nth-child(16):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -168px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(66.66667%, rgba(0, 0, 0, 0)), color-stop(66.66667%, #000000), color-stop(100%, #000000)) 0 -168px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 -168px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 -168px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 -168px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ョ";
  padding-top: 216px;
  overflow: hidden;
  -webkit-animation: 1920ms column-16 infinite steps(1, start);
  -moz-animation: 1920ms column-16 infinite steps(1, start);
  -ms-animation: 1920ms column-16 infinite steps(1, start);
  -o-animation: 1920ms column-16 infinite steps(1, start);
  animation: 1920ms column-16 infinite steps(1, start);
}

@-webkit-keyframes column-16 {
  0% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
}
@-moz-keyframes column-16 {
  0% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
}
@-o-keyframes column-16 {
  0% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
}
@keyframes column-16 {
  0% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 72px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
}
.matrix li:nth-child(17) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(17):before {
  display: block;
  width: 1em;
  content: "ョ し わ コ ょ こ バ シ ソ ョ ョ マ ラ ソ ウ ト シ マ う シ タ タ フ ょ";
}
.matrix li:nth-child(17):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 96px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(66.66667%, rgba(0, 0, 0, 0)), color-stop(66.66667%, #000000), color-stop(100%, #000000)) 0 96px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 96px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 96px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 384px, #000000 384px, #000000 576px) 0 96px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "タ";
  padding-top: 480px;
  overflow: hidden;
  -webkit-animation: 1920ms column-17 infinite steps(1, start);
  -moz-animation: 1920ms column-17 infinite steps(1, start);
  -ms-animation: 1920ms column-17 infinite steps(1, start);
  -o-animation: 1920ms column-17 infinite steps(1, start);
  animation: 1920ms column-17 infinite steps(1, start);
}

@-webkit-keyframes column-17 {
  0% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 456px;
  }
}
@-moz-keyframes column-17 {
  0% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 456px;
  }
}
@-o-keyframes column-17 {
  0% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 456px;
  }
}
@keyframes column-17 {
  0% {
    background-position: 0 96px;
    padding-top: 480px;
  }
  4.16667% {
    background-position: 0 120px;
    padding-top: 504px;
  }
  8.33333% {
    background-position: 0 144px;
    padding-top: 528px;
  }
  12.5% {
    background-position: 0 168px;
    padding-top: 552px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 0px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 24px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 48px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 72px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 96px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 120px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 144px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 168px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 192px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 216px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 240px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 264px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 288px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 312px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 336px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 360px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 384px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 408px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 432px;
  }
  95.83333% {
    background-position: 0 72px;
    padding-top: 456px;
  }
}
.matrix li:nth-child(18) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(18):before {
  display: block;
  width: 1em;
  content: "バ ラ し ョ ば と ド ソ ソ ょ ウ た タ フ ば ウ ん シ ン ソ ン タ き ョ";
}
.matrix li:nth-child(18):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -264px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(91.66667%, rgba(0, 0, 0, 0)), color-stop(91.66667%, #000000), color-stop(100%, #000000)) 0 -264px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -264px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -264px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -264px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "た";
  padding-top: 264px;
  overflow: hidden;
  -webkit-animation: 1920ms column-18 infinite steps(1, start);
  -moz-animation: 1920ms column-18 infinite steps(1, start);
  -ms-animation: 1920ms column-18 infinite steps(1, start);
  -o-animation: 1920ms column-18 infinite steps(1, start);
  animation: 1920ms column-18 infinite steps(1, start);
}

@-webkit-keyframes column-18 {
  0% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
}
@-moz-keyframes column-18 {
  0% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
}
@-o-keyframes column-18 {
  0% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
}
@keyframes column-18 {
  0% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  4.16667% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  8.33333% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  12.5% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  16.66667% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  20.83333% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  25% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  29.16667% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  33.33333% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  37.5% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
  41.66667% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  45.83333% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  50% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  54.16667% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  58.33333% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  62.5% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  66.66667% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  70.83333% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  75% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  79.16667% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  83.33333% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  87.5% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  91.66667% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  95.83333% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
}
.matrix li:nth-child(19) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(19):before {
  display: block;
  width: 1em;
  content: "ん ウ バ リ わ ソ き と ば た バ ト ば こ ょ タ ば フ バ わ し ょ ョ ょ";
}
.matrix li:nth-child(19):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(91.66667%, rgba(0, 0, 0, 0)), color-stop(91.66667%, #000000), color-stop(100%, #000000)) 0 -24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 528px, #000000 528px, #000000 576px) 0 -24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ょ";
  padding-top: 504px;
  overflow: hidden;
  -webkit-animation: 1920ms column-19 infinite steps(1, start);
  -moz-animation: 1920ms column-19 infinite steps(1, start);
  -ms-animation: 1920ms column-19 infinite steps(1, start);
  -o-animation: 1920ms column-19 infinite steps(1, start);
  animation: 1920ms column-19 infinite steps(1, start);
}

@-webkit-keyframes column-19 {
  0% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
}
@-moz-keyframes column-19 {
  0% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
}
@-o-keyframes column-19 {
  0% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
}
@keyframes column-19 {
  0% {
    background-position: 0 -24px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 480px;
  }
}
.matrix li:nth-child(20) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(20):before {
  display: block;
  width: 1em;
  content: "キ キ ょ バ た ソ こ ウ う キ た わ ョ と ン ウ ウ ド と と う ょ ン バ";
}
.matrix li:nth-child(20):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -72px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(29.16667%, rgba(0, 0, 0, 0)), color-stop(29.16667%, #000000), color-stop(100%, #000000)) 0 -72px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 -72px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 -72px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 168px, #000000 168px, #000000 576px) 0 -72px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "た";
  padding-top: 96px;
  overflow: hidden;
  -webkit-animation: 1920ms column-20 infinite steps(1, start);
  -moz-animation: 1920ms column-20 infinite steps(1, start);
  -ms-animation: 1920ms column-20 infinite steps(1, start);
  -o-animation: 1920ms column-20 infinite steps(1, start);
  animation: 1920ms column-20 infinite steps(1, start);
}

@-webkit-keyframes column-20 {
  0% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
}
@-moz-keyframes column-20 {
  0% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
}
@-o-keyframes column-20 {
  0% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
}
@keyframes column-20 {
  0% {
    background-position: 0 -72px;
    padding-top: 96px;
  }
  4.16667% {
    background-position: 0 -48px;
    padding-top: 120px;
  }
  8.33333% {
    background-position: 0 -24px;
    padding-top: 144px;
  }
  12.5% {
    background-position: 0 0px;
    padding-top: 168px;
  }
  16.66667% {
    background-position: 0 24px;
    padding-top: 192px;
  }
  20.83333% {
    background-position: 0 48px;
    padding-top: 216px;
  }
  25% {
    background-position: 0 72px;
    padding-top: 240px;
  }
  29.16667% {
    background-position: 0 96px;
    padding-top: 264px;
  }
  33.33333% {
    background-position: 0 120px;
    padding-top: 288px;
  }
  37.5% {
    background-position: 0 144px;
    padding-top: 312px;
  }
  41.66667% {
    background-position: 0 168px;
    padding-top: 336px;
  }
  45.83333% {
    background-position: 0 192px;
    padding-top: 360px;
  }
  50% {
    background-position: 0 216px;
    padding-top: 384px;
  }
  54.16667% {
    background-position: 0 240px;
    padding-top: 408px;
  }
  58.33333% {
    background-position: 0 264px;
    padding-top: 432px;
  }
  62.5% {
    background-position: 0 288px;
    padding-top: 456px;
  }
  66.66667% {
    background-position: 0 312px;
    padding-top: 480px;
  }
  70.83333% {
    background-position: 0 336px;
    padding-top: 504px;
  }
  75% {
    background-position: 0 360px;
    padding-top: 528px;
  }
  79.16667% {
    background-position: 0 384px;
    padding-top: 552px;
  }
  83.33333% {
    background-position: 0 -168px;
    padding-top: 0px;
  }
  87.5% {
    background-position: 0 -144px;
    padding-top: 24px;
  }
  91.66667% {
    background-position: 0 -120px;
    padding-top: 48px;
  }
  95.83333% {
    background-position: 0 -96px;
    padding-top: 72px;
  }
}
.matrix li:nth-child(21) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(21):before {
  display: block;
  width: 1em;
  content: "こ ば う た こ フ こ ウ ウ た シ ょ タ ん う リ ン シ ソ コ キ ば た ラ";
}
.matrix li:nth-child(21):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -288px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(62.5%, rgba(0, 0, 0, 0)), color-stop(62.5%, #000000), color-stop(100%, #000000)) 0 -288px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 360px, #000000 360px, #000000 576px) 0 -288px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 360px, #000000 360px, #000000 576px) 0 -288px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 360px, #000000 360px, #000000 576px) 0 -288px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "た";
  padding-top: 72px;
  overflow: hidden;
  -webkit-animation: 1920ms column-21 infinite steps(1, start);
  -moz-animation: 1920ms column-21 infinite steps(1, start);
  -ms-animation: 1920ms column-21 infinite steps(1, start);
  -o-animation: 1920ms column-21 infinite steps(1, start);
  animation: 1920ms column-21 infinite steps(1, start);
}

@-webkit-keyframes column-21 {
  0% {
    background-position: 0 -288px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 144px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 168px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 192px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 48px;
  }
}
@-moz-keyframes column-21 {
  0% {
    background-position: 0 -288px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 144px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 168px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 192px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 48px;
  }
}
@-o-keyframes column-21 {
  0% {
    background-position: 0 -288px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 144px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 168px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 192px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 48px;
  }
}
@keyframes column-21 {
  0% {
    background-position: 0 -288px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -264px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -240px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -216px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -192px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -168px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -144px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -120px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -96px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -72px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -48px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -24px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 0px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 24px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 48px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 96px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 120px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 144px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 168px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 192px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -360px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -336px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -312px;
    padding-top: 48px;
  }
}
.matrix li:nth-child(22) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(22):before {
  display: block;
  width: 1em;
  content: "と こ た タ ン ワ ワ ン こ し う ト リ う し マ わ ソ う ラ ラ き う リ";
}
.matrix li:nth-child(22):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 240px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(54.16667%, rgba(0, 0, 0, 0)), color-stop(54.16667%, #000000), color-stop(100%, #000000)) 0 240px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 240px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 240px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 312px, #000000 312px, #000000 576px) 0 240px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "リ";
  padding-top: 552px;
  overflow: hidden;
  -webkit-animation: 1920ms column-22 infinite steps(1, start);
  -moz-animation: 1920ms column-22 infinite steps(1, start);
  -ms-animation: 1920ms column-22 infinite steps(1, start);
  -o-animation: 1920ms column-22 infinite steps(1, start);
  animation: 1920ms column-22 infinite steps(1, start);
}

@-webkit-keyframes column-22 {
  0% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 528px;
  }
}
@-moz-keyframes column-22 {
  0% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 528px;
  }
}
@-o-keyframes column-22 {
  0% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 528px;
  }
}
@keyframes column-22 {
  0% {
    background-position: 0 240px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -312px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -288px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 528px;
  }
}
.matrix li:nth-child(23) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(23):before {
  display: block;
  width: 1em;
  content: "ン バ こ き た ラ マ シ シ バ ょ コ シ ン ン う う ョ う う た し こ ン";
}
.matrix li:nth-child(23):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -360px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(87.5%, rgba(0, 0, 0, 0)), color-stop(87.5%, #000000), color-stop(100%, #000000)) 0 -360px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -360px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -360px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -360px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "マ";
  padding-top: 144px;
  overflow: hidden;
  -webkit-animation: 1920ms column-23 infinite steps(1, start);
  -moz-animation: 1920ms column-23 infinite steps(1, start);
  -ms-animation: 1920ms column-23 infinite steps(1, start);
  -o-animation: 1920ms column-23 infinite steps(1, start);
  animation: 1920ms column-23 infinite steps(1, start);
}

@-webkit-keyframes column-23 {
  0% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
}
@-moz-keyframes column-23 {
  0% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
}
@-o-keyframes column-23 {
  0% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
}
@keyframes column-23 {
  0% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  4.16667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  8.33333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  12.5% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  16.66667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  20.83333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  75% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  79.16667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  83.33333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  87.5% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  91.66667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  95.83333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
}
.matrix li:nth-child(24) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(24):before {
  display: block;
  width: 1em;
  content: "コ ワ ょ こ バ ウ リ ウ ソ た タ と ょ ド し ン ラ ョ ラ し ん ウ タ ョ";
}
.matrix li:nth-child(24):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -384px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(75%, rgba(0, 0, 0, 0)), color-stop(75%, #000000), color-stop(100%, #000000)) 0 -384px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -384px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -384px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 432px, #000000 432px, #000000 576px) 0 -384px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ょ";
  padding-top: 48px;
  overflow: hidden;
  -webkit-animation: 1920ms column-24 infinite steps(1, start);
  -moz-animation: 1920ms column-24 infinite steps(1, start);
  -ms-animation: 1920ms column-24 infinite steps(1, start);
  -o-animation: 1920ms column-24 infinite steps(1, start);
  animation: 1920ms column-24 infinite steps(1, start);
}

@-webkit-keyframes column-24 {
  0% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
}
@-moz-keyframes column-24 {
  0% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
}
@-o-keyframes column-24 {
  0% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
}
@keyframes column-24 {
  0% {
    background-position: 0 -384px;
    padding-top: 48px;
  }
  4.16667% {
    background-position: 0 -360px;
    padding-top: 72px;
  }
  8.33333% {
    background-position: 0 -336px;
    padding-top: 96px;
  }
  12.5% {
    background-position: 0 -312px;
    padding-top: 120px;
  }
  16.66667% {
    background-position: 0 -288px;
    padding-top: 144px;
  }
  20.83333% {
    background-position: 0 -264px;
    padding-top: 168px;
  }
  25% {
    background-position: 0 -240px;
    padding-top: 192px;
  }
  29.16667% {
    background-position: 0 -216px;
    padding-top: 216px;
  }
  33.33333% {
    background-position: 0 -192px;
    padding-top: 240px;
  }
  37.5% {
    background-position: 0 -168px;
    padding-top: 264px;
  }
  41.66667% {
    background-position: 0 -144px;
    padding-top: 288px;
  }
  45.83333% {
    background-position: 0 -120px;
    padding-top: 312px;
  }
  50% {
    background-position: 0 -96px;
    padding-top: 336px;
  }
  54.16667% {
    background-position: 0 -72px;
    padding-top: 360px;
  }
  58.33333% {
    background-position: 0 -48px;
    padding-top: 384px;
  }
  62.5% {
    background-position: 0 -24px;
    padding-top: 408px;
  }
  66.66667% {
    background-position: 0 0px;
    padding-top: 432px;
  }
  70.83333% {
    background-position: 0 24px;
    padding-top: 456px;
  }
  75% {
    background-position: 0 48px;
    padding-top: 480px;
  }
  79.16667% {
    background-position: 0 72px;
    padding-top: 504px;
  }
  83.33333% {
    background-position: 0 96px;
    padding-top: 528px;
  }
  87.5% {
    background-position: 0 120px;
    padding-top: 552px;
  }
  91.66667% {
    background-position: 0 -432px;
    padding-top: 0px;
  }
  95.83333% {
    background-position: 0 -408px;
    padding-top: 24px;
  }
}
.matrix li:nth-child(25) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(25):before {
  display: block;
  width: 1em;
  content: "ラ ト ょ ラ と ょ ワ し ン う う ク ン ば わ た ょ ワ コ ン し ン リ ソ";
}
.matrix li:nth-child(25):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(16.66667%, rgba(0, 0, 0, 0)), color-stop(16.66667%, #000000), color-stop(100%, #000000)) 0 -24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 -24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 -24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 96px, #000000 96px, #000000 576px) 0 -24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ラ";
  padding-top: 72px;
  overflow: hidden;
  -webkit-animation: 1920ms column-25 infinite steps(1, start);
  -moz-animation: 1920ms column-25 infinite steps(1, start);
  -ms-animation: 1920ms column-25 infinite steps(1, start);
  -o-animation: 1920ms column-25 infinite steps(1, start);
  animation: 1920ms column-25 infinite steps(1, start);
}

@-webkit-keyframes column-25 {
  0% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
}
@-moz-keyframes column-25 {
  0% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
}
@-o-keyframes column-25 {
  0% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
}
@keyframes column-25 {
  0% {
    background-position: 0 -24px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 24px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 48px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 72px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 96px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 120px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 144px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 168px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 192px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 216px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 240px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 264px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 288px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 312px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 336px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 360px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 384px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 408px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 432px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 456px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 48px;
  }
}
.matrix li:nth-child(26) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(26):before {
  display: block;
  width: 1em;
  content: "わ ウ ん マ た リ と ん ウ ソ ん う ン た マ タ ウ し マ シ ソ タ き う";
}
.matrix li:nth-child(26):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 216px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(37.5%, rgba(0, 0, 0, 0)), color-stop(37.5%, #000000), color-stop(100%, #000000)) 0 216px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 216px, #000000 216px, #000000 576px) 0 216px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 216px, #000000 216px, #000000 576px) 0 216px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 216px, #000000 216px, #000000 576px) 0 216px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "マ";
  padding-top: 432px;
  overflow: hidden;
  -webkit-animation: 1920ms column-26 infinite steps(1, start);
  -moz-animation: 1920ms column-26 infinite steps(1, start);
  -ms-animation: 1920ms column-26 infinite steps(1, start);
  -o-animation: 1920ms column-26 infinite steps(1, start);
  animation: 1920ms column-26 infinite steps(1, start);
}

@-webkit-keyframes column-26 {
  0% {
    background-position: 0 216px;
    padding-top: 432px;
  }
  4.16667% {
    background-position: 0 240px;
    padding-top: 456px;
  }
  8.33333% {
    background-position: 0 264px;
    padding-top: 480px;
  }
  12.5% {
    background-position: 0 288px;
    padding-top: 504px;
  }
  16.66667% {
    background-position: 0 312px;
    padding-top: 528px;
  }
  20.83333% {
    background-position: 0 336px;
    padding-top: 552px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 0px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 24px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 48px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 72px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 96px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 120px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 144px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 168px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 192px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 216px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 240px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 264px;
  }
  75% {
    background-position: 0 72px;
    padding-top: 288px;
  }
  79.16667% {
    background-position: 0 96px;
    padding-top: 312px;
  }
  83.33333% {
    background-position: 0 120px;
    padding-top: 336px;
  }
  87.5% {
    background-position: 0 144px;
    padding-top: 360px;
  }
  91.66667% {
    background-position: 0 168px;
    padding-top: 384px;
  }
  95.83333% {
    background-position: 0 192px;
    padding-top: 408px;
  }
}
@-moz-keyframes column-26 {
  0% {
    background-position: 0 216px;
    padding-top: 432px;
  }
  4.16667% {
    background-position: 0 240px;
    padding-top: 456px;
  }
  8.33333% {
    background-position: 0 264px;
    padding-top: 480px;
  }
  12.5% {
    background-position: 0 288px;
    padding-top: 504px;
  }
  16.66667% {
    background-position: 0 312px;
    padding-top: 528px;
  }
  20.83333% {
    background-position: 0 336px;
    padding-top: 552px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 0px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 24px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 48px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 72px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 96px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 120px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 144px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 168px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 192px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 216px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 240px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 264px;
  }
  75% {
    background-position: 0 72px;
    padding-top: 288px;
  }
  79.16667% {
    background-position: 0 96px;
    padding-top: 312px;
  }
  83.33333% {
    background-position: 0 120px;
    padding-top: 336px;
  }
  87.5% {
    background-position: 0 144px;
    padding-top: 360px;
  }
  91.66667% {
    background-position: 0 168px;
    padding-top: 384px;
  }
  95.83333% {
    background-position: 0 192px;
    padding-top: 408px;
  }
}
@-o-keyframes column-26 {
  0% {
    background-position: 0 216px;
    padding-top: 432px;
  }
  4.16667% {
    background-position: 0 240px;
    padding-top: 456px;
  }
  8.33333% {
    background-position: 0 264px;
    padding-top: 480px;
  }
  12.5% {
    background-position: 0 288px;
    padding-top: 504px;
  }
  16.66667% {
    background-position: 0 312px;
    padding-top: 528px;
  }
  20.83333% {
    background-position: 0 336px;
    padding-top: 552px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 0px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 24px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 48px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 72px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 96px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 120px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 144px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 168px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 192px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 216px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 240px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 264px;
  }
  75% {
    background-position: 0 72px;
    padding-top: 288px;
  }
  79.16667% {
    background-position: 0 96px;
    padding-top: 312px;
  }
  83.33333% {
    background-position: 0 120px;
    padding-top: 336px;
  }
  87.5% {
    background-position: 0 144px;
    padding-top: 360px;
  }
  91.66667% {
    background-position: 0 168px;
    padding-top: 384px;
  }
  95.83333% {
    background-position: 0 192px;
    padding-top: 408px;
  }
}
@keyframes column-26 {
  0% {
    background-position: 0 216px;
    padding-top: 432px;
  }
  4.16667% {
    background-position: 0 240px;
    padding-top: 456px;
  }
  8.33333% {
    background-position: 0 264px;
    padding-top: 480px;
  }
  12.5% {
    background-position: 0 288px;
    padding-top: 504px;
  }
  16.66667% {
    background-position: 0 312px;
    padding-top: 528px;
  }
  20.83333% {
    background-position: 0 336px;
    padding-top: 552px;
  }
  25% {
    background-position: 0 -216px;
    padding-top: 0px;
  }
  29.16667% {
    background-position: 0 -192px;
    padding-top: 24px;
  }
  33.33333% {
    background-position: 0 -168px;
    padding-top: 48px;
  }
  37.5% {
    background-position: 0 -144px;
    padding-top: 72px;
  }
  41.66667% {
    background-position: 0 -120px;
    padding-top: 96px;
  }
  45.83333% {
    background-position: 0 -96px;
    padding-top: 120px;
  }
  50% {
    background-position: 0 -72px;
    padding-top: 144px;
  }
  54.16667% {
    background-position: 0 -48px;
    padding-top: 168px;
  }
  58.33333% {
    background-position: 0 -24px;
    padding-top: 192px;
  }
  62.5% {
    background-position: 0 0px;
    padding-top: 216px;
  }
  66.66667% {
    background-position: 0 24px;
    padding-top: 240px;
  }
  70.83333% {
    background-position: 0 48px;
    padding-top: 264px;
  }
  75% {
    background-position: 0 72px;
    padding-top: 288px;
  }
  79.16667% {
    background-position: 0 96px;
    padding-top: 312px;
  }
  83.33333% {
    background-position: 0 120px;
    padding-top: 336px;
  }
  87.5% {
    background-position: 0 144px;
    padding-top: 360px;
  }
  91.66667% {
    background-position: 0 168px;
    padding-top: 384px;
  }
  95.83333% {
    background-position: 0 192px;
    padding-top: 408px;
  }
}
.matrix li:nth-child(27) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(27):before {
  display: block;
  width: 1em;
  content: "た ョ ウ ば う ょ ん ド ょ ソ ワ タ シ シ き バ ド ド ワ ラ ウ ウ う こ";
}
.matrix li:nth-child(27):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 240px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(45.83333%, rgba(0, 0, 0, 0)), color-stop(45.83333%, #000000), color-stop(100%, #000000)) 0 240px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 240px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 240px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 264px, #000000 264px, #000000 576px) 0 240px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ウ";
  padding-top: 504px;
  overflow: hidden;
  -webkit-animation: 1920ms column-27 infinite steps(1, start);
  -moz-animation: 1920ms column-27 infinite steps(1, start);
  -ms-animation: 1920ms column-27 infinite steps(1, start);
  -o-animation: 1920ms column-27 infinite steps(1, start);
  animation: 1920ms column-27 infinite steps(1, start);
}

@-webkit-keyframes column-27 {
  0% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 480px;
  }
}
@-moz-keyframes column-27 {
  0% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 480px;
  }
}
@-o-keyframes column-27 {
  0% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 480px;
  }
}
@keyframes column-27 {
  0% {
    background-position: 0 240px;
    padding-top: 504px;
  }
  4.16667% {
    background-position: 0 264px;
    padding-top: 528px;
  }
  8.33333% {
    background-position: 0 288px;
    padding-top: 552px;
  }
  12.5% {
    background-position: 0 -264px;
    padding-top: 0px;
  }
  16.66667% {
    background-position: 0 -240px;
    padding-top: 24px;
  }
  20.83333% {
    background-position: 0 -216px;
    padding-top: 48px;
  }
  25% {
    background-position: 0 -192px;
    padding-top: 72px;
  }
  29.16667% {
    background-position: 0 -168px;
    padding-top: 96px;
  }
  33.33333% {
    background-position: 0 -144px;
    padding-top: 120px;
  }
  37.5% {
    background-position: 0 -120px;
    padding-top: 144px;
  }
  41.66667% {
    background-position: 0 -96px;
    padding-top: 168px;
  }
  45.83333% {
    background-position: 0 -72px;
    padding-top: 192px;
  }
  50% {
    background-position: 0 -48px;
    padding-top: 216px;
  }
  54.16667% {
    background-position: 0 -24px;
    padding-top: 240px;
  }
  58.33333% {
    background-position: 0 0px;
    padding-top: 264px;
  }
  62.5% {
    background-position: 0 24px;
    padding-top: 288px;
  }
  66.66667% {
    background-position: 0 48px;
    padding-top: 312px;
  }
  70.83333% {
    background-position: 0 72px;
    padding-top: 336px;
  }
  75% {
    background-position: 0 96px;
    padding-top: 360px;
  }
  79.16667% {
    background-position: 0 120px;
    padding-top: 384px;
  }
  83.33333% {
    background-position: 0 144px;
    padding-top: 408px;
  }
  87.5% {
    background-position: 0 168px;
    padding-top: 432px;
  }
  91.66667% {
    background-position: 0 192px;
    padding-top: 456px;
  }
  95.83333% {
    background-position: 0 216px;
    padding-top: 480px;
  }
}
.matrix li:nth-child(28) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(28):before {
  display: block;
  width: 1em;
  content: "ウ う ょ ョ ば た フ タ わ マ ソ マ わ ワ き バ し リ リ シ ラ タ ょ し";
}
.matrix li:nth-child(28):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -432px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(87.5%, rgba(0, 0, 0, 0)), color-stop(87.5%, #000000), color-stop(100%, #000000)) 0 -432px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -432px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -432px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -432px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ョ";
  padding-top: 72px;
  overflow: hidden;
  -webkit-animation: 1920ms column-28 infinite steps(1, start);
  -moz-animation: 1920ms column-28 infinite steps(1, start);
  -ms-animation: 1920ms column-28 infinite steps(1, start);
  -o-animation: 1920ms column-28 infinite steps(1, start);
  animation: 1920ms column-28 infinite steps(1, start);
}

@-webkit-keyframes column-28 {
  0% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
}
@-moz-keyframes column-28 {
  0% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
}
@-o-keyframes column-28 {
  0% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
}
@keyframes column-28 {
  0% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  87.5% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
  91.66667% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  95.83333% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
}
.matrix li:nth-child(29) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(29):before {
  display: block;
  width: 1em;
  content: "シ ょ う ウ ク シ ン ソ き ん ド キ シ キ こ た た ク シ ョ た ょ ラ ト";
}
.matrix li:nth-child(29):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 144px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(70.83333%, rgba(0, 0, 0, 0)), color-stop(70.83333%, #000000), color-stop(100%, #000000)) 0 144px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 408px, #000000 408px, #000000 576px) 0 144px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 408px, #000000 408px, #000000 576px) 0 144px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 408px, #000000 408px, #000000 576px) 0 144px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ト";
  padding-top: 552px;
  overflow: hidden;
  -webkit-animation: 1920ms column-29 infinite steps(1, start);
  -moz-animation: 1920ms column-29 infinite steps(1, start);
  -ms-animation: 1920ms column-29 infinite steps(1, start);
  -o-animation: 1920ms column-29 infinite steps(1, start);
  animation: 1920ms column-29 infinite steps(1, start);
}

@-webkit-keyframes column-29 {
  0% {
    background-position: 0 144px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 72px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 96px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 120px;
    padding-top: 528px;
  }
}
@-moz-keyframes column-29 {
  0% {
    background-position: 0 144px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 72px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 96px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 120px;
    padding-top: 528px;
  }
}
@-o-keyframes column-29 {
  0% {
    background-position: 0 144px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 72px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 96px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 120px;
    padding-top: 528px;
  }
}
@keyframes column-29 {
  0% {
    background-position: 0 144px;
    padding-top: 552px;
  }
  4.16667% {
    background-position: 0 -408px;
    padding-top: 0px;
  }
  8.33333% {
    background-position: 0 -384px;
    padding-top: 24px;
  }
  12.5% {
    background-position: 0 -360px;
    padding-top: 48px;
  }
  16.66667% {
    background-position: 0 -336px;
    padding-top: 72px;
  }
  20.83333% {
    background-position: 0 -312px;
    padding-top: 96px;
  }
  25% {
    background-position: 0 -288px;
    padding-top: 120px;
  }
  29.16667% {
    background-position: 0 -264px;
    padding-top: 144px;
  }
  33.33333% {
    background-position: 0 -240px;
    padding-top: 168px;
  }
  37.5% {
    background-position: 0 -216px;
    padding-top: 192px;
  }
  41.66667% {
    background-position: 0 -192px;
    padding-top: 216px;
  }
  45.83333% {
    background-position: 0 -168px;
    padding-top: 240px;
  }
  50% {
    background-position: 0 -144px;
    padding-top: 264px;
  }
  54.16667% {
    background-position: 0 -120px;
    padding-top: 288px;
  }
  58.33333% {
    background-position: 0 -96px;
    padding-top: 312px;
  }
  62.5% {
    background-position: 0 -72px;
    padding-top: 336px;
  }
  66.66667% {
    background-position: 0 -48px;
    padding-top: 360px;
  }
  70.83333% {
    background-position: 0 -24px;
    padding-top: 384px;
  }
  75% {
    background-position: 0 0px;
    padding-top: 408px;
  }
  79.16667% {
    background-position: 0 24px;
    padding-top: 432px;
  }
  83.33333% {
    background-position: 0 48px;
    padding-top: 456px;
  }
  87.5% {
    background-position: 0 72px;
    padding-top: 480px;
  }
  91.66667% {
    background-position: 0 96px;
    padding-top: 504px;
  }
  95.83333% {
    background-position: 0 120px;
    padding-top: 528px;
  }
}
.matrix li:nth-child(30) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(30):before {
  display: block;
  width: 1em;
  content: "ン わ し ワ タ き わ キ タ わ ょ シ ラ う シ リ リ リ う リ ラ タ ト バ";
}
.matrix li:nth-child(30):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -120px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(58.33333%, rgba(0, 0, 0, 0)), color-stop(58.33333%, #000000), color-stop(100%, #000000)) 0 -120px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -120px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -120px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 336px, #000000 336px, #000000 576px) 0 -120px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "わ";
  padding-top: 216px;
  overflow: hidden;
  -webkit-animation: 1920ms column-30 infinite steps(1, start);
  -moz-animation: 1920ms column-30 infinite steps(1, start);
  -ms-animation: 1920ms column-30 infinite steps(1, start);
  -o-animation: 1920ms column-30 infinite steps(1, start);
  animation: 1920ms column-30 infinite steps(1, start);
}

@-webkit-keyframes column-30 {
  0% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
}
@-moz-keyframes column-30 {
  0% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
}
@-o-keyframes column-30 {
  0% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
}
@keyframes column-30 {
  0% {
    background-position: 0 -120px;
    padding-top: 216px;
  }
  4.16667% {
    background-position: 0 -96px;
    padding-top: 240px;
  }
  8.33333% {
    background-position: 0 -72px;
    padding-top: 264px;
  }
  12.5% {
    background-position: 0 -48px;
    padding-top: 288px;
  }
  16.66667% {
    background-position: 0 -24px;
    padding-top: 312px;
  }
  20.83333% {
    background-position: 0 0px;
    padding-top: 336px;
  }
  25% {
    background-position: 0 24px;
    padding-top: 360px;
  }
  29.16667% {
    background-position: 0 48px;
    padding-top: 384px;
  }
  33.33333% {
    background-position: 0 72px;
    padding-top: 408px;
  }
  37.5% {
    background-position: 0 96px;
    padding-top: 432px;
  }
  41.66667% {
    background-position: 0 120px;
    padding-top: 456px;
  }
  45.83333% {
    background-position: 0 144px;
    padding-top: 480px;
  }
  50% {
    background-position: 0 168px;
    padding-top: 504px;
  }
  54.16667% {
    background-position: 0 192px;
    padding-top: 528px;
  }
  58.33333% {
    background-position: 0 216px;
    padding-top: 552px;
  }
  62.5% {
    background-position: 0 -336px;
    padding-top: 0px;
  }
  66.66667% {
    background-position: 0 -312px;
    padding-top: 24px;
  }
  70.83333% {
    background-position: 0 -288px;
    padding-top: 48px;
  }
  75% {
    background-position: 0 -264px;
    padding-top: 72px;
  }
  79.16667% {
    background-position: 0 -240px;
    padding-top: 96px;
  }
  83.33333% {
    background-position: 0 -216px;
    padding-top: 120px;
  }
  87.5% {
    background-position: 0 -192px;
    padding-top: 144px;
  }
  91.66667% {
    background-position: 0 -168px;
    padding-top: 168px;
  }
  95.83333% {
    background-position: 0 -144px;
    padding-top: 192px;
  }
}
.matrix li:nth-child(31) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(31):before {
  display: block;
  width: 1em;
  content: "ン ク ド わ た と う ウ ン ョ ド シ ラ シ し ば フ コ リ ド フ う ク ん";
}
.matrix li:nth-child(31):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -480px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(87.5%, rgba(0, 0, 0, 0)), color-stop(87.5%, #000000), color-stop(100%, #000000)) 0 -480px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -480px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -480px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 504px, #000000 504px, #000000 576px) 0 -480px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ク";
  padding-top: 24px;
  overflow: hidden;
  -webkit-animation: 1920ms column-31 infinite steps(1, start);
  -moz-animation: 1920ms column-31 infinite steps(1, start);
  -ms-animation: 1920ms column-31 infinite steps(1, start);
  -o-animation: 1920ms column-31 infinite steps(1, start);
  animation: 1920ms column-31 infinite steps(1, start);
}

@-webkit-keyframes column-31 {
  0% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  4.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  8.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  12.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  95.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
}
@-moz-keyframes column-31 {
  0% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  4.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  8.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  12.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  95.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
}
@-o-keyframes column-31 {
  0% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  4.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  8.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  12.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  95.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
}
@keyframes column-31 {
  0% {
    background-position: 0 -480px;
    padding-top: 24px;
  }
  4.16667% {
    background-position: 0 -456px;
    padding-top: 48px;
  }
  8.33333% {
    background-position: 0 -432px;
    padding-top: 72px;
  }
  12.5% {
    background-position: 0 -408px;
    padding-top: 96px;
  }
  16.66667% {
    background-position: 0 -384px;
    padding-top: 120px;
  }
  20.83333% {
    background-position: 0 -360px;
    padding-top: 144px;
  }
  25% {
    background-position: 0 -336px;
    padding-top: 168px;
  }
  29.16667% {
    background-position: 0 -312px;
    padding-top: 192px;
  }
  33.33333% {
    background-position: 0 -288px;
    padding-top: 216px;
  }
  37.5% {
    background-position: 0 -264px;
    padding-top: 240px;
  }
  41.66667% {
    background-position: 0 -240px;
    padding-top: 264px;
  }
  45.83333% {
    background-position: 0 -216px;
    padding-top: 288px;
  }
  50% {
    background-position: 0 -192px;
    padding-top: 312px;
  }
  54.16667% {
    background-position: 0 -168px;
    padding-top: 336px;
  }
  58.33333% {
    background-position: 0 -144px;
    padding-top: 360px;
  }
  62.5% {
    background-position: 0 -120px;
    padding-top: 384px;
  }
  66.66667% {
    background-position: 0 -96px;
    padding-top: 408px;
  }
  70.83333% {
    background-position: 0 -72px;
    padding-top: 432px;
  }
  75% {
    background-position: 0 -48px;
    padding-top: 456px;
  }
  79.16667% {
    background-position: 0 -24px;
    padding-top: 480px;
  }
  83.33333% {
    background-position: 0 0px;
    padding-top: 504px;
  }
  87.5% {
    background-position: 0 24px;
    padding-top: 528px;
  }
  91.66667% {
    background-position: 0 48px;
    padding-top: 552px;
  }
  95.83333% {
    background-position: 0 -504px;
    padding-top: 0px;
  }
}
.matrix li:nth-child(32) {
  position: relative;
  display: inline-block;
  white-space: normal;
  vertical-align: top;
}
.matrix li:nth-child(32):before {
  display: block;
  width: 1em;
  content: "フ ワ タ と ょ タ ラ コ う ラ た う ン ク ウ き う し タ キ ン リ ソ ょ";
}
.matrix li:nth-child(32):after {
  -moz-box-sizing: border-box;
  -webkit-box-sizing: border-box;
  box-sizing: border-box;
  background: url('data:image/svg+xml;base64,') 0 -24px repeat;
  background: -webkit-gradient(linear, 50% 0%, 50% 576, color-stop(0%, #000000), color-stop(95.83333%, rgba(0, 0, 0, 0)), color-stop(95.83333%, #000000), color-stop(100%, #000000)) 0 -24px repeat;
  background: -moz-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 552px, #000000 552px, #000000 576px) 0 -24px repeat;
  background: -webkit-linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 552px, #000000 552px, #000000 576px) 0 -24px repeat;
  background: linear-gradient(#000000 0%, rgba(0, 0, 0, 0) 552px, #000000 552px, #000000 576px) 0 -24px repeat;
  display: block;
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  color: white;
  content: "ソ";
  padding-top: 528px;
  overflow: hidden;
  -webkit-animation: 1920ms column-32 infinite steps(1, start);
  -moz-animation: 1920ms column-32 infinite steps(1, start);
  -ms-animation: 1920ms column-32 infinite steps(1, start);
  -o-animation: 1920ms column-32 infinite steps(1, start);
  animation: 1920ms column-32 infinite steps(1, start);
}

@-webkit-keyframes column-32 {
  0% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
}
@-moz-keyframes column-32 {
  0% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
}
@-o-keyframes column-32 {
  0% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 504px;
  }
}
@keyframes column-32 {
  0% {
    background-position: 0 -24px;
    padding-top: 528px;
  }
  4.16667% {
    background-position: 0 0px;
    padding-top: 552px;
  }
  8.33333% {
    background-position: 0 -552px;
    padding-top: 0px;
  }
  12.5% {
    background-position: 0 -528px;
    padding-top: 24px;
  }
  16.66667% {
    background-position: 0 -504px;
    padding-top: 48px;
  }
  20.83333% {
    background-position: 0 -480px;
    padding-top: 72px;
  }
  25% {
    background-position: 0 -456px;
    padding-top: 96px;
  }
  29.16667% {
    background-position: 0 -432px;
    padding-top: 120px;
  }
  33.33333% {
    background-position: 0 -408px;
    padding-top: 144px;
  }
  37.5% {
    background-position: 0 -384px;
    padding-top: 168px;
  }
  41.66667% {
    background-position: 0 -360px;
    padding-top: 192px;
  }
  45.83333% {
    background-position: 0 -336px;
    padding-top: 216px;
  }
  50% {
    background-position: 0 -312px;
    padding-top: 240px;
  }
  54.16667% {
    background-position: 0 -288px;
    padding-top: 264px;
  }
  58.33333% {
    background-position: 0 -264px;
    padding-top: 288px;
  }
  62.5% {
    background-position: 0 -240px;
    padding-top: 312px;
  }
  66.66667% {
    background-position: 0 -216px;
    padding-top: 336px;
  }
  70.83333% {
    background-position: 0 -192px;
    padding-top: 360px;
  }
  75% {
    background-position: 0 -168px;
    padding-top: 384px;
  }
  79.16667% {
    background-position: 0 -144px;
    padding-top: 408px;
  }
  83.33333% {
    background-position: 0 -120px;
    padding-top: 432px;
  }
  87.5% {
    background-position: 0 -96px;
    padding-top: 456px;
  }
  91.66667% {
    background-position: 0 -72px;
    padding-top: 480px;
  }
  95.83333% {
    background-position: 0 -48px;
    padding-top: 504px;
    }
}

</style>